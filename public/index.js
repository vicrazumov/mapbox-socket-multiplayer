...
map.on('load', function() {
  ...
  const showPopup = (coordinates, description) => popup
  ...
  // add socket handlers
  socket.on(MAP_MOVE, center => map.setCenter(center));
  socket.on(MAP_CLICK_FEATURE, featureId => showPopup(features[featureId].coordinates, features[featureId].description));
  socket.on(MAP_ZOOM_UPDATE, zoom => {
    myself = false;
    map.setZoom(zoom);
  });
  socket.on(MAP_CLOSE_POPUP, () => {
    myself = false;
    popup.remove();
  });
});